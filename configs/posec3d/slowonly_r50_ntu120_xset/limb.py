...
data = dict(
    ...
    train=dict(
        type='RepeatDataset',
        times=10,
        dataset=dict(type=dataset_type, ann_file=ann_file, split='xset_train', pipeline=train_pipeline)),
    val=dict(type=dataset_type, ann_file=ann_file, split='xset_val', pipeline=val_pipeline),
    test=dict(type=dataset_type, ann_file=ann_file, split='xset_val', pipeline=test_pipeline))
# optimizer
...
log_level = 'INFO'
work_dir = './work_dirs/posec3d/slowonly_r50_ntu120_xset/limb'
